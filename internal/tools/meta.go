...
type objectReply struct {
	...
	// чем в основной таблице (Обороты.Счет, а не СчетДт; ОборотыДтКт.СуммаОборот, а не
	// СуммаОборотДт). Раньше отдавалось правило их построения, и правило нужно было
	// применить — на этом шаге слабая модель и ошибалась (прогон 31.08.2026, три отказа
	// из пяти). Готовый перечень применять не нужно.
	ВиртуальныеТаблицы []struct {
		Имя       string   `json:"имя"`
		Поля      []string `json:"поля"`
		Параметры []string `json:"параметры"`
	} `json:"виртуальныеТаблицы"`
...
func (s *Set) Object(ctx context.Context, _ *mcp.CallToolRequest, in ObjectInput) (*mcp.CallToolResult, any, error) {
	...
	if len(reply.ВиртуальныеТаблицы) > 0 {
		// Готовый перечень вместо правила его построения: правило нужно ПРИМЕНИТЬ, а именно
		// на применении слабая модель и ошибается — в прогоне 31.08.2026 три отказа из пяти
		// были обращением к Обороты.СчетДт и ОборотыДтКт.СуммаОборотДт, которых у этих
		// таблиц нет. Из списка достаточно выбрать.
		b.WriteString("\nВиртуальные таблицы регистра — ИМЕНА ПОЛЕЙ ЗДЕСЬ ДРУГИЕ, чем в основной;\n" +
			"берите их отсюда, а не достраивайте по образцу:\n")
		for _, vt := range reply.ВиртуальныеТаблицы {
			fmt.Fprintf(&b, "\n  %s.%s", reply.ПолноеИмя, vt.Имя)
			if len(vt.Параметры) > 0 {
				fmt.Fprintf(&b, "(%s)", strings.Join(vt.Параметры, ", "))
			}
			fmt.Fprintf(&b, "\n    поля: %s\n", strings.Join(vt.Поля, ", "))
		}
	} else if len(reply.Ресурсы) > 0 {
