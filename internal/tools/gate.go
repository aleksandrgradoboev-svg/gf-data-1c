...
//
// Состояние живёт в сессии, а не в процессе. В режиме stdio сессия одна и совпадает с
// процессом. В сетевом режиме (-http) сервер собирается фабрикой на КАЖДУЮ НОВУЮ сессию
// (go-sdk: getServer зовётся только при sessInfo == nil), поэтому у каждого подключения
// свой гейт, и одобренный текст не течёт между клиентами. Проверено поведением 04.09.2026.
