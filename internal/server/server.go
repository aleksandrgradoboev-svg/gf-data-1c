...
у любого инструмента. Вызов без base уходит в базу по умолчанию, поэтому, когда баз
больше одной, называй её явно: молчаливый уход не в ту базу выглядит как достоверный
ответ. Незнакомое имя базы даёт отказ с перечнем известных, а не пустой результат.
