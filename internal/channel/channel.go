...
func (c *Client) classifyStatus(code int, body []byte) error {
	...
	case code == http.StatusNotFound:
		...
		// а его происхождение: HTML-страницу «404 Not Found» рисует сам Apache,
		// когда до 1С запрос не дошёл (нет такой публикации), а 1С на неизвестный
		// маршрут отвечает пустым телом без Content-Type. Замерено на живом
		// стенде 03.09.2026; прежде оба случая назывались «нет расширения»,
		// и человек искал расширение там, где не было опубликовано базы.
		if bytes.Contains(body, []byte("<html")) || bytes.Contains(body, []byte("<HTML")) {
			return refusal.New(refusal.NoPublication, "базы нет по этому адресу",
				"HTTP 404 страницей веб-сервера — до 1С запрос не дошёл",
				"проверьте имя публикации в адресе: "+c.base.URL,
		...
		return refusal.New(refusal.NoExtension, "расширение не отвечает в этой базе",
			...
			"расширение доступа к данным не установлено в базе "+c.base.Name+" или выключено",
	...
	case code == http.StatusUnauthorized || code == http.StatusForbidden:
		...
		// РУКАМИ через Конфигуратор: установщик объявляет роль расширения
		// основной сам, а при ручной загрузке cfe роль остаётся невыданной.
		...
		return refusal.New(refusal.Unauthorized, "база отказала в доступе",
			fmt.Sprintf("HTTP %d — какая из причин, платформа не сообщает", code),
			"либо в базе нет пользователя "+c.base.User+", либо неверен пароль: "+
				"поправить — bases с action=update (пустой пароль не трогается)",
