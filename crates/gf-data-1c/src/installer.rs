...
mod tests {
    ...
    #[test]
    fn встроенное_расширение_полно_если_собрано() {
        // Расширение кладётся в бинарь отдельным шагом сборки, и каталог для него —
        // артефакт, которого в репозитории нет. Поэтому проверка УСЛОВНАЯ: сборка без
        // расширения законна (так собирается CI и свежий клон), и требовать его здесь
        // значит красить сборку в красный за отсутствие того, чего в репозитории нет.
        // Ровно на этом упал CI слоя 12 — 04.09.2026, обе платформы.
        //
        // Проверяется не наличие, а СОГЛАСОВАННОСТЬ: если расширение объявлено
        // встроенным, главный файл выгрузки обязан читаться. Что бинарь без расширения
        // говорит об этом вслух, проверяет соседний тест.
        if !extension_built() {
            return; // законная сборка без артефакта — проверять нечего
        }
        assert!(
            EXTENSION.get_file("Configuration.xml").is_some(),
            "расширение объявлено встроенным, но Configuration.xml не читается"
        );
    }
